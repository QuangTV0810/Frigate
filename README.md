...
```bash
git clone https://github.com/QuangTV0810/Frigate.git
cd Frigate
```
...
### 3. Khởi chạy Frigate và add camera trên UI
...
Điều này có nghĩa là repo hiện không hardcode camera trong file config. Camera được add trực tiếp trên giao diện web UI của Frigate sau khi container chạy lên.
...
- `cameras: {}`: repo hiện không hardcode camera trong file config; camera được add trực tiếp trên UI.
